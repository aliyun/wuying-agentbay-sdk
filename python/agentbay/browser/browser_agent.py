...
class BrowserAgent(BaseService):
    ...
    def act(
        ...
        try:
            ...
            if response.success:
                logger.debug(
                    f"Response from CallMcpTool - page_use_act: {response.data}"
                )
    ...
    def observe(
        ...
        try:
            ...
            if response.success:
                logger.debug(
                    f"Response from CallMcpTool - page_use_observe: {response.data}"
                )
                import json as _json
                ...
                logger.error(
                    f"Response from CallMcpTool - page_use_observe: {response.error_message}"
                )
                return False, []
            ...
            if response.success:
                ...
                for item in observeResults:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Could not parse arguments as JSON: {arguments_str}"
                        )
    ...
    def extract(self, options: ExtractOptions, page=None) -> Tuple[bool, T]:
        ...
        try:
            ...
            if response.success:
                logger.debug(
                    f"Response from CallMcpTool - page_use_extract: {response.data}"
                )
                import json as _json
                ...
                logger.error(
                    f"Response from CallMcpTool - page_use_extract: {response.error_message}"
                )
                return False, None
        except Exception as e:
